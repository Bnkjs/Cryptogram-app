...
router.get('/', authorization, async (req,res)=>{
      ...
      try {
        //1. find user
        const checkUserExist = await pool.query('SELECT email,username,avatar,balance FROM users WHERE user_id = ($1)',[req.user])
        ...
        if(checkUserExist.rows[0] === undefined){
            ...
            res.json({ user: checkUserExist.rows[0], contact: userContact.rows[0] ,order: userOrderInfos.rows, transfert: userTransfertInfos.rows, investment: userInvestmentInfos.rows})
        }
      } catch (error) {
